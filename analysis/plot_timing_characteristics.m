...
for nchs = 1:64
    for nfreqs = 1:64
        ...
        nsets_skip1(nchs,nfreqs) = ndsets;

        % exhaustive patterns
        [allpats_t_vs_freq(nchs,nfreqs),~,nsets_exhaustive(nchs,nfreqs)] = get_acquisition_time(nchs,nfreqs,0,navgs,nsamps);
    end
end

%--------------------------------------------------------------------------
% Plot single frequency acquisition vs channel --- THESIS FIGURE 
%--------------------------------------------------------------------------
chs = 1:64;
...
daq_t_curve(1,:) = nsets_skip1(:,1).*daq_t_vs_freq(:,1);  % acquisition time
t_curve2(1,:) = allpats_t_vs_freq(:,1);
lb_y_curve = min(daq_t_curve) *ones(size(chs)); % lower bound y curve
semilogy(chs,t_curve,'k','LineWidth',2); hold on
semilogy(chs(3:end),t_curve2(3:end),'b','LineWidth',2); hold on
semilogy(chs,daq_t_curve,'b');

% Fill in between curves
fillx = [chs, fliplr(chs)];
% filly = [t_curve, fliplr(t_curve2)];
% fill(fillx,filly,'b','FaceAlpha',0.8)
...
legend('total time (1 skip)','total time (exhaustive)','','data tx time','data acq time','30 fps target','Location','northeastoutside')
...
%%
%--------------------------------------------------------------------------
% Get timing for single mux set vs number of datasets acquired vs. nfft
%--------------------------------------------------------------------------
clearvars total_t daq_t ndsets daq_t_curve lb_y_curve
nchs = 4; 
nfreqs = 1; 
nskips = 0; % exhaustive patterns
navgs = 1;
nfft = 2.^[0:11];
for n = 1:length(nfft)
    nsamps = nfft(n);
...
%--------------------------------------------------------------------------
% Plot tetrapolar acquisition time vs nfft --- THESIS FIGURE 
%--------------------------------------------------------------------------
...
ylabel('Acquisition Time (s)')
title('Acquisition Time for Exhaustive Tetrapolar Measurements')
%--------------------------------------------------------------------------
% Get timing vs number of measurments --- THESIS FIGURE 
%--------------------------------------------------------------------------
subplot 122
fps_target = 1/30 *ones(size(nsets_skip1(:,1))); % 30 fps target
plot(nsets_skip1(:,1),t_vs_freq(:,1)); hold on
plot(nsets_skip1(:,1),fps_target,'k--');
grid on
legend('total acq time','30 fps target','Location','northeastoutside')
