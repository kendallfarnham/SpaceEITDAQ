...
%-----------------------------------------------------------------------
function ret_msg = main_gui_plot_elec_snr(plt_handle,iis,vvs,adc_data_struct)
...
for n_ii = 1:numel(iis)/2
    ...
    xdata1 = adc_data_struct(n_ii,1).snr_i; % get first vv pattern
    xdata2 = adc_data_struct(n_ii,1).snr_v1;
    xdata3 = adc_data_struct(n_ii,1).snr_v2;
    for n_vv = 2:length(vvs)
        xdata1 = [xdata1; adc_data_struct(n_ii,n_vv).snr_i];
        xdata2 = [xdata2; adc_data_struct(n_ii,n_vv).snr_v1];
        xdata3 = [xdata3; adc_data_struct(n_ii,n_vv).snr_v2];
    end
...
title(t,'SNR per Vs+/Vs- pair');
xlabel(t,'Measurement no.'); ylabel(t,'SNR (dB)')


drawnow
ret_msg = 'Finished plotting snr data.';